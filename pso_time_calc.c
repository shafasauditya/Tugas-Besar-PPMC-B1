...
double tourDistance(int tour[], Coordinate cities[], int dim) {
    ...
    totalDistance += distance(cities[tour[dim - 1]], cities[tour[0]]); // Connect back to starting city
...
void PSO_TSP(Coordinate cities[], int dim) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        for (int i = 0; i < POPULATION_SIZE; i++) {
            ...
            // membuat array velocity agar semua kota dikunjungi sekali
            int visited[dim];
            for (int j = 0; j < dim; j++) {
                visited[j] = 0;
            }
...
int main() {
    ...
    double totaltime = 0;
    double worst_time = 0;
    double best_time = 999999999999999999999999.0;
    for (int i = 0; i < 5; i++) { 
        clock_t time1 = clock();
        PSO_TSP(cities, dim);
        clock_t time2 = clock();
        double elapsed = (double)(time2 - time1) / CLOCKS_PER_SEC;
        printf("Time elapsed for iteration %d: %lf\n", i + 1, elapsed);
    ...
    double average_time = totaltime / 5;
    printf("Average time elapsed: %lf\n", average_time);
    printf("Worst time elapsed: %lf\n", worst_time);
    printf("Best time elapsed: %lf\n", best_time);
    // membebaskan memori
    free(cities);
    return 0;
}
